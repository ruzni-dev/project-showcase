...
const minimumPreloaderTime = 900;
...
function hidePreloader() {
    ...
    const waitTime = Math.max(0, minimumPreloaderTime - elapsed);
    window.setTimeout(() => {
...
let _currentScroll = { rafId: null, cancel: false };
function cancelScroll() {
    if (_currentScroll.rafId) {
        cancelAnimationFrame(_currentScroll.rafId);
    }
    _currentScroll.cancel = true;
    _currentScroll.rafId = null;
}

function smoothScrollTo(targetY, duration) {
    // cancel any in-progress scroll
    cancelScroll();
    _currentScroll.cancel = false;
    const startY = window.scrollY || document.documentElement.scrollTop;
    const distance = targetY - startY;
    const defaultDuration = Math.min(1200, Math.max(250, Math.abs(distance) * 0.35));
    ...
    function step(now) {
        if (_currentScroll.cancel) return;
        const elapsed = now - startTime;
        const t = Math.min(1, elapsed / dur);
        const eased = easeInOutCubic(t);
        window.scrollTo(0, Math.round(startY + distance * eased));
        if (elapsed < dur) {
            _currentScroll.rafId = requestAnimationFrame(step);
        } else {
            _currentScroll.rafId = null;
        }
    }
...
function openChatDemo() {
    ...
    smoothScrollTo(document.body.scrollHeight, 900);
}
...
if (goTopBtn) {
    goTopBtn.addEventListener('click', (e) => {
        e.preventDefault();
        // cancel any running scroll and start a quick smooth scroll to top
        cancelScroll();
