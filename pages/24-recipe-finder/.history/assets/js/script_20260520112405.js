...
const recipes = [
    ...
    },
    {
        ...
        instructions: [
            ...
            "Poach the eggs until the whites are set and yolks remain runny.",
            "Spread avocado over the toast and top with the poached eggs.",
            "Finish with chili flakes and a light drizzle of olive oil."
        ]
    },
    {
        id: 9,
        ...
        instructions: [
            ...
            "Brush with teriyaki sauce and let it glaze for 2 minutes.",
            "Slice the chicken and arrange over the rice.",
            "Add broccoli and carrots on the side.",
    ...
    },
    {
        ...
        instructions: [
            ...
            "Mix olive oil, garlic, dill, lemon juice, salt, and pepper.",
    ...
    },
    {
        ...
        instructions: [
            ...
            "Stir-fry the vegetables in sesame oil until crisp-tender.",
            "Add ginger, soy sauce, and hoisin sauce.",
            "Toss in the noodles and combine well.",
            "Serve hot."
        ]
    }
];
...
const recipeMeta = {
    ...
    8: { dietary: ["Vegetarian"], tags: ["Breakfast", "Quick", "Light"] },
    9: { dietary: ["High-Protein"], tags: ["Lunch", "Balanced"] },
    10: { dietary: ["Vegetarian", "Gluten-Free"], tags: ["Breakfast", "Dessert", "Light"] },
    11: { dietary: ["Gluten-Free", "Pescatarian"], tags: ["Dinner", "Protein", "Healthy"], featured: true },
    12: { dietary: ["Vegan", "Vegetarian"], tags: ["Quick", "Weeknight", "Plant-Based"] }
};
...
function setActiveFilter(filterLabel) {
    const filterButtons = Array.from(document.querySelectorAll('.filter-btn'));
    filterButtons.forEach(button => button.classList.remove('active'));

    const matchedButton = filterButtons.find(button => button.textContent.trim() === filterLabel);
    if (matchedButton) {
...
function getCollectionCards() {
    return [
        ...
        },
        {
            ...
            description: 'Jump back into bookmarked recipes',
            icon: 'fa-bookmark',
            action: () => {
        ...
        },
        {
            id: 'light',
            title: 'Light meals',
            description: 'Fresh picks under 400 calories',
            icon: 'fa-leaf',
            action: () => {
                sortSelect.value = 'calories';
                setActiveFilter('Light Meals');
                handleSearch();
            }
        },
        {
            id: 'dessert',
            title: 'Sweet endings',
            description: 'Dessert-first recipes for weekends',
            icon: 'fa-ice-cream',
            action: () => {
...
function renderCollections() {
    ...
    collectionsGrid.innerHTML = collectionCards.map(card => {
        const count = card.id === 'quick'
            ? recipes.filter(recipe => recipe.time <= 30).length
            : card.id === 'favorites'
                ? recipeFavorites.size
                : card.id === 'light'
                    ? recipes.filter(recipe => recipe.calories <= 400).length
                    : recipes.filter(recipe => recipe.cuisine === 'French' || getRecipeMeta(recipe).tags.includes('Dessert')).length;

        return `
            <article class="collection-card" data-collection-id="${card.id}">
                <div class="collection-icon"><i class="fas ${card.icon}"></i></div>
                <div class="collection-copy">
                    <span class="collection-count">${count} recipes</span>
                    <h3>${card.title}</h3>
                    <p>${card.description}</p>
                </div>
                <button type="button" class="collection-action">Open</button>
            </article>
        `;
    }).join('');

    collectionsGrid.querySelectorAll('[data-collection-id]').forEach(element => {
        const card = collectionCards.find(item => item.id === element.dataset.collectionId);
        element.addEventListener('click', () => {
            if (card) {
                card.action();
                showToast(`${card.title} applied`);
            }
        });
    });
}
